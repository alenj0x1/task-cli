...
// interface RespuestaConError {
//     ok: false;
//     error: string;
// }


// const respuesta: RespuestaCorrecta | RespuestaConError = {
//     ok: true,
//     data: "hola",
//     timestamp: new Date()
// }
...
function consultarUsuarios(): ApiResponse<UsuarioPrueba[]> {
    return {
        errors: [],
        data: [
            {
...
interface UsuarioComponente {
    name: string;
}

const usuariosConNombreYApellidos: UsuarioComponente[] = usuarios.data.map(usuario => {
    return {
        name: `${usuario.nombre} ${usuario.apellido}`
    }
})
...
function parsearPlantilla(plantilla: string, codigo: string)
{
